...
void    ImGui_ImplDX11_InvalidateDeviceObjects()
{
    ...
    if (g_pFontTextureView) { g_pFontTextureView->Release(); g_pFontTextureView = NULL; } // We copied g_pFontTextureView to io.Fonts->TexID so let's clear that as well.
    if (ImGui::GetIO().Fonts != nullptr) { ImGui::GetIO().Fonts->TexID = NULL; }
    if (g_pIB) { g_pIB->Release(); g_pIB = NULL; }
    if (g_pVB) { g_pVB->Release(); g_pVB = NULL; }
